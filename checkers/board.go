...
const (
	...
	UNFINISHED GAME_RESULT = "Unfinished"
)
...
func (b Board) GameResult() GAME_RESULT {
	...
	if b.roundsAfterLastKill > DRAW_MOVES {
		...
		blackMoves := len(b.GetAllMovesFor(BLACK))
		whiteMoves := len(b.GetAllMovesFor(WHITE))

		b.boardLogger.Printf("Draw because no kills were made for some time, moves left - black: %d, whites: %d\n", blackMoves, whiteMoves)
		return DRAW
	}
